...
from credit_risk.utils.constants import (
...
from credit_risk.utils.utils import cap_outliers, preprocess_data, save_object
...
class DataTransformation:
    ...
    def initiate_data_transformation(self, X_train_path, X_test_path, y_train_path, y_test_path):
        try:
            ...
            X_train, low_perc, high_perc = cap_outliers(
                X_train,
                percentile_low=OUTLIER_CAPPING_LOWER_THRESHOLD,
                percentile_high=OUTLIER_CAPPING_UPPER_THRESHOLD,
                req_columns=OUTLIER_COLUMNS,
            )
            X_test[OUTLIER_COLUMNS] = X_test[OUTLIER_COLUMNS].clip(
                lower=low_perc, upper=high_perc, axis=1
            )

            logging.info("Saving outlier thresholds in JSON format")
            thresholds = {"low_perc": low_perc.to_dict(), "high_perc": high_perc.to_dict()}
