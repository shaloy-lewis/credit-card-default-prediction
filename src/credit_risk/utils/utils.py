...
def cap_outliers(df, percentile_low=2.5, percentile_high=97.5, req_columns=None):
    try:
...
def preprocess_data(df):
    try:
        ...
        for i in [0, 2, 3, 4, 5, 6]:
            df[f"PAY_{i}"] = np.where(
                df[f"PAY_{i}"] < 0,
                "bill_paid",
                (np.where(df[f"PAY_{i}"] > 0, "bill_payment_delay", "revolving_credit")),
            )

        df["EDUCATION"] = np.where(df["EDUCATION"].isin([0, 4, 5, 6]), 4, df["EDUCATION"])
        df["EDUCATION"] = df["EDUCATION"].map(
            {1: "graduate_school", 2: "university", 3: "high_school", 4: "others"}
        )
