...
COPY pyproject.toml uv.lock README.md LICENSE ./
COPY src ./src
COPY configs/artifacts/hf_distribution_v2.lock.json ./configs/artifacts/hf_distribution_v2.lock.json
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    && uv run --no-sync credit-risk artifacts pull \
    && uv run --no-sync credit-risk artifacts verify


FROM python:3.12.14-slim-bookworm@sha256:782412e85d0f0984994c290652577d4018aff08145c85b262bb63dc0c7522254 AS runtime

RUN apt-get update \
    && apt-get install --no-install-recommends --yes libpcre2-8-0=10.42-1+deb12u1 \
