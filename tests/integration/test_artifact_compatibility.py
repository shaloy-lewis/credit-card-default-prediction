...
def test_committed_artifacts_load_and_preserve_documented_prediction(
    legacy_prediction_payload: dict[str, int | float | str],
) -> None:
    ...
    features = CustomData(**legacy_prediction_payload).get_data_as_dataframe()
...
def test_custom_data_preserves_input_column_order(
    ...
    assert features.columns.tolist() == list(legacy_prediction_payload)
