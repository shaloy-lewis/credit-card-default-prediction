...
def write_artifacts(
    ...
    *,
    ...
    return write_manifest(artifact_dir)


def write_manifest(artifact_dir: Path) -> Path:
    files: dict[str, dict[str, object]] = {}
    for filename, serialization in (
        ("model.pkl", "python_pickle"),
        ("preprocessor.pkl", "python_pickle"),
        ("outlier_threshold.json", "json"),
    ):
        path = artifact_dir / filename
        payload = path.read_bytes() if path.exists() else b""
        files[filename] = {
            "size_bytes": max(1, len(payload)),
            "sha256": hashlib.sha256(payload).hexdigest(),
            "serialization": serialization,
        }
    manifest = artifact_dir.with_name(f"{artifact_dir.name}-legacy-manifest.json")
    manifest.write_text(
        json.dumps(
            {
...
def test_load_artifact_bundle_accepts_matching_artifacts(tmp_path: Path) -> None:
    ...
    bundle = load_artifact_bundle(tmp_path, manifest_path=manifest)
...
def test_load_artifact_bundle_rejects_missing_threshold_section(tmp_path: Path) -> None:
    ...
    manifest = write_artifacts(tmp_path, thresholds=thresholds)
...
def test_load_artifact_bundle_requires_model_methods(tmp_path: Path) -> None:
    manifest = write_artifacts(tmp_path, model=object())
...
def test_load_artifact_bundle_rejects_non_binary_or_reordered_classes(tmp_path: Path) -> None:
    ...
    manifest = write_artifacts(tmp_path, model=model)
...
def test_digest_mismatch_prevents_pickle_execution(tmp_path: Path, monkeypatch) -> None:
    manifest = write_artifacts(tmp_path)
    (tmp_path / "model.pkl").write_bytes(b"changed")
    called = False

    def unsafe_load(_file_obj):
        nonlocal called
        called = True
        raise AssertionError("pickle must not execute")

    monkeypatch.setattr(pickle, "load", unsafe_load)
    with pytest.raises(ArtifactValidationError, match="Do not deserialize"):
        load_artifact_bundle(tmp_path, manifest_path=manifest)
